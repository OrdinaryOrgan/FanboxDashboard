...
def test_download_public_link_recovery_ignores_existing_unrelated_archives(monkeypatch, tmp_path: Path) -> None:
    downloads_dir = tmp_path / "downloads"
    downloads_dir.mkdir(parents=True, exist_ok=True)
    with ZipFile(downloads_dir / "old.zip", "w") as archive:
    ...
    async def fake_create_subprocess_exec(*args, **kwargs):
        ...
        async def write_zip() -> None:
            await asyncio.sleep(0.02)
            with ZipFile(destination / "expectedC.zip", "w") as archive:
                archive.writestr("sample.txt", b"hello")
    ...
    assert Path(archive_path).name == "expectedC.zip"
    assert "Recovered downloaded archive after MEGAcmd did not exit cleanly" in output


def test_download_public_link_accepts_overwritten_matching_archive(monkeypatch, tmp_path: Path) -> None:
    downloads_dir = tmp_path / "downloads"
    downloads_dir.mkdir(parents=True, exist_ok=True)
    existing_zip = downloads_dir / "expectedC.zip"
    with ZipFile(existing_zip, "w") as archive:
        archive.writestr("old.txt", b"old")
    ...
    assert Path(archive_path) == existing_zip
    assert output == ""
